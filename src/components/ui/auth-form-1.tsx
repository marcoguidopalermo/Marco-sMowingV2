...
import logo from "@/public/logo/LOGOBLACK.png"; // Using black logo for light background form
...
function Auth({ className, onSignInSuccess, onSignUpSuccess, onGoogleLogin, ...props }: React.ComponentProps<"div"> & { onSignInSuccess?: (email: string, pass: string) => void; onSignUpSuccess?: (name: string, email: string, pass: string) => void; onGoogleLogin?: () => void }) {
  ...
  return (
    <div
      ...
      {...props}
    >
      <div className="relative overflow-hidden rounded-xl border border-border/50 bg-white/90 shadow-xl backdrop-blur-sm">
...
function AuthSocialButtons({ isLoading, onGoogleLogin }: AuthSocialButtonsProps) {
  return (
    <div data-slot="auth-social-buttons" className="w-full mt-6">
      <Button
        variant="outline"
        className="w-full h-12 bg-background/50 border-border/50 hover:bg-slate-50 transition-colors"
        disabled={isLoading}
        onClick={(e) => {
          e.preventDefault();
          onGoogleLogin?.();
...
function AuthSignIn({ onForgotPassword, onSignUp, onSubmitSuccess, onGoogleLogin }: AuthSignInProps) {
  ...
  const onSubmit = async (data: SignInFormValues) => {
    ...
    try {
      ...
      setFormState((prev) => ({ ...prev, error: err.message || "An unexpected error occurred" }));
    } finally {
  ...
  return (
    ...
    >
      ...
      <AuthForm onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <Button
              type="button"
              variant="link"
              className="h-auto p-0 text-xs text-lime-600 hover:text-lime-700"
        ...
        <Button type="submit" className="w-full bg-lime-600 hover:bg-lime-700 text-white" disabled={formState.isLoading}>
          {formState.isLoading ? (
            <>
      ...
      <p className="mt-8 text-center text-sm text-muted-foreground">
        ...
        <Button
          variant="link"
          className="h-auto p-0 text-sm text-lime-600 hover:text-lime-700 font-bold"
          onClick={onSignUp}
          disabled={formState.isLoading}
        >
          Create one
        ...
        <div className="flex items-center space-x-2">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              ...
              <Button variant="link" className="h-auto p-0 text-xs text-lime-600">
                Terms
              </Button>{" "}
              and{" "}
              <Button variant="link" className="h-auto p-0 text-xs text-lime-600">
  ...
  return (
    ...
    >
      <Button
        variant="ghost"
        size="icon"
        className="absolute left-4 top-4 hover:bg-slate-50"
        onClick={onSignIn}
        disabled={formState.isLoading}
      >
...
function AuthResetSuccess({ onSignIn }: AuthResetSuccessProps) {
  return (
    ...
    >
      <div className="mb-6 flex h-16 w-16 items-center justify-center rounded-full bg-lime-100">
        <MailCheck className="h-8 w-8 text-lime-600" />
      </div>
      ...
      <Button
        variant="outline"
        className="mt-6 w-full max-w-xs border-lime-600 text-lime-600 hover:bg-lime-50"
        onClick={onSignIn}
      >
        Back to sign in
      </Button>
