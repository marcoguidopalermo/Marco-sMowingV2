...
// manager explicitly removed from this crew's perf entry. Drop-in
// helpers (workers/managers whose hours land via the split or
// "Add Unscheduled Employee" flows) live in employeeAH only and
// are intentionally NOT counted toward the bracket here.
export function scheduledCrewSize(
  ...
  return (crew.employees || []).filter(id => !removed.has(id)).length;
}
...
export function getCrewAllowance(
  ...
  const size = scheduledCrewSize(crew, log || null);
  const pct = pctForSize(size, settings?.crewSizeAllowance);
  return { size, pct, source: 'live' };
}
