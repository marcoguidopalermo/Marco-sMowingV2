...
import {
  priceSnow, measureGrid, SNOW_PRICING_CONFIG, SNOW_PRICING_CONFIG_VERSION, SnowGrid,
} from './snowPricing';
...
test('All cells dragged with DRAG_COUNTS_TOWARD_SIZE=false → falls back, still prices', () => {
  const original = SNOW_PRICING_CONFIG.DRAG_COUNTS_TOWARD_SIZE;
  SNOW_PRICING_CONFIG.DRAG_COUNTS_TOWARD_SIZE = false;
  try {
    const allDrag: number[][] = Array.from({ length: ROWS }, () => Array(COLS).fill(2));
    const p = priceSnow(allDrag);
    ...
    assert.equal(p!.floor, SNOW_PRICING_CONFIG.CUSTOM_FLOOR + ROWS * COLS * SNOW_PRICING_CONFIG.DRAG_RATE);
  } finally {
    SNOW_PRICING_CONFIG.DRAG_COUNTS_TOWARD_SIZE = original;
  }
});
